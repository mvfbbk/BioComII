...
CREATE TABLE genbankTosequenceToexon(gb_accession VARCHAR(8) NOT NULL DEFAULT 'N/A', seq_accession VARCHAR(8)   NOT NULL DEFAULT 'N/A', PRIMARY KEY CLUSTERED (gb_accession, seq_accession), FOREIGN KEY (gb_accession) REFERENCES genbank ( accession_number ) ON UPDATE  NO ACTION  ON DELETE  CASCADE, FOREIGN KEY (seq_accession) REFERENCES sequences ( accession_number ) ON UPDATE  NO ACTION  ON DELETE  CASCADE);
